---
title: Arena: keywords.rules bodies — hook group C — battle: blocking, counters, attack, damage and battle end
...
stage: 7
---
**Source:** plan Stage 7; the hook contract issue (this depends on it); hooks in this group: **block, counterWindow, onAttackDeclared, beforeDamage, battleEnd**; `src/lib/arena/glossary.ts` for each keyword's meaning and the engine's current approximation; rule manual §22; `scripts/verify/keywords.ts`.

**Candidate keywords** (confirm against the inventory — a keyword belongs to the group whose hooks its body needs, and some need two groups; move it to the later one): [Blocker], [Attack], [Revenge], [Super Combo], [Awaken] (the Leader flip on damage), [Ultimate], [Double/Triple Strike] where printed as parameters.
