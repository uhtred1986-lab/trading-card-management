...
**Steps in order (this issue).** Not started: `CostRecord` (`src/lib/arena/engine/script.ts` line 452) has `orbs, either, marker, burst, spiritBoost, text, condition, program` and nothing about other payers.
1. Type: `CostRecord.payWith?: { sel: Selector; as: "energy" | { orb: Color } }[]`; `lang/validate.ts`'s `costProblem` (line 53) checks it; `printCost` (`lang/print.ts` line 297) and the `COST` branch of `parse.ts` (grep `parseCost`) get one more item form — `docs/arena-rules-language.md` §3 `item` production gains `"PAYWITH" SEL "AS" (energy | "{" colour "}")`.
2. Engine: `planPayment` in `engine/state.ts` (grep) takes the eligible payers into its candidate set; the `payCost` prompt's `options: Payment[]` (`types.ts` line 615) already carries a list of card ids per option — extend `Payment` only if a payer is not an energy card id today.
3. A [Permanent] "you can use this card as energy" is a `ContinuousEffect` kind `"payer"` (add to `types.ts` line 376) that `planPayment` consults.
4. Wording: `src/lib/arena/wording.ts` price sentence; `pay` beat names the card used (`beats.ts`, grep `"pay"`).
5. `scripts/verify/lang.ts` round-trips the new item minimal and maximal; `verify/keywords.ts` pays a skill with the card and charges nothing from energy; `contract:emit`; glossary.
