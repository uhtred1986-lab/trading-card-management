...
- **"for the duration of this turn"** is "for the duration of the turn" with a
  ...
  action pattern anchors on `# Arena rules engine — the work list
...
("Battle **Cards**" sets no type) — both widenings, both on the list.
 ...
 after those tails come off, so one word left
  BT3-013's "+10000 combo power" and BT1-003's negate unreadable. `durationOf`
  learned "duration of this battle/game" at the same time, for the same
  reason.
...
Area. Each needs `splitClauses` to keep more together *and* `refsFor`/
