...
function tally(defs: CardDef[]): Tally {
...
const all = (await db.select().from(cards)).map(cardDefFrom);
console.log(line("whole catalog", tally(all)));
...
console.log(`                ${pct(deckPermCompiled, deckPerm)} of ${deckPerm} [Permanent] skills compile — but the engine has no static-effect layer yet, so none of them apply.`);

console.log("\nclauses the parser could not read, most common first:");
for (const [clause, n] of [...misses.entries()].sort((a, b) => b[1] - a[1]).slice(0, 30)) console.log(`  ${String(n).padStart(5)}×  ${clause}`);
