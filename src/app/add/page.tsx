    ...
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      {open.length ? (
        <Link href="/add/scan" className="block rounded-xl border border-ki-500/40 bg-ki-500/5 p-3 text-sm hover:border-ki-500">
          ...
          <span className="text-space-300">
            — {open.reduce((n, b) => n + b.items, 0)} cards waiting for review, {open.reduce((n, b) => n + b.needsReview, 0)} need a look. Continue →
          </span>
        </Link>
      ) : null}
      <Link href="/add/quick" className="block rounded-xl border-2 border-ki-500/60 bg-ki-500/5 p-4 hover:border-ki-500">
        ...
        <p className="mt-1 text-sm text-space-300">
          One card at a time, phone-first: snap it, it&apos;s identified on the spot, tap the number of copies, save — the camera opens again for the next card.
        </p>
      </Link>
      <div className="grid gap-3 sm:grid-cols-2">
        <Link href="/add/scan" className="rounded-xl border border-space-700/70 bg-space-900/60 p-4 hover:border-ki-500/50">
          ...
          <p className="mt-1 text-sm text-space-300">
            Photograph one card or a whole binder page. Every match is reviewed before it&apos;s saved — upload from the phone, finish on the PC. Photos are kept only until the batch is confirmed.
          </p>
        </Link>
