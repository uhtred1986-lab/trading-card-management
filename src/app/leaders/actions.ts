 ...
 * towards is exactly the case where you don't have the leader yet.
 */
export async function searchLeadersAction(q: string): Promise<LeaderChoice[]> {
  ...
  return leaders.map((l) => ({ id: l.id, name: l.name, setCode: l.setCode, colors: l.colors, imageUrl: l.imageUrl, owned: owned.get(l.id) ?? 0 }));
}
