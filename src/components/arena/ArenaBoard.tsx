 ...
 * On a laptop the same layout scales up through `--arena` (globals.css), and a
 * mouse gets a full-size preview of whatever card it is over.
 *
...
export function ArenaBoard({
  ...
  const panel = "rounded-xl border border-space-700/70 bg-space-900/60 p-2 sm:rounded-2xl sm:p-3 lg:p-4";

  return (
    <div className="arena relative mx-auto flex min-h-[calc(100dvh-8rem)] w-full max-w-5xl flex-col gap-2 sm:gap-3 xl:max-w-6xl">
      <PhaseStrip view={view} />

      {/* The opponent's side, compressed. */}
      <section className={panel} aria-label={`${view.them.name}'s side`}>
        <SideSummary side={view.them} them />
        <div className="mt-2 flex items-end gap-1.5 overflow-x-auto pb-1 sm:gap-2 lg:gap-3">
          ...
          {view.them.battle.length === 0 && <span className="self-center text-xs text-space-500 lg:text-sm">no Battle Cards</span>}
          <span className="ml-auto flex gap-[2px] self-center sm:gap-1">
            {Array.from({ length: Math.min(view.them.handCount, 8) }, (_, i) => (
              <span
                key={i}
                className="h-[calc(2rem*var(--arena,1))] w-[calc(18px*var(--arena,1))] rounded-[2px] border border-space-600 bg-[radial-gradient(circle_at_50%_50%,#f28c0f_0_10%,#0f1220_11%)]"
              />
            ))}
          </span>
        </div>
        <EnergyRow side={view.them} onHover={hoverOf} small />
      </section>

      {/* The middle band: both Combo Areas and the power figures during a battle. */}
      {view.battle ? (
        <section className="rounded-xl border border-ki-500/35 bg-gradient-to-b from-ki-500/10 to-transparent p-2 sm:rounded-2xl sm:p-3" aria-label="Battle">
          <div className="flex items-baseline justify-center gap-3 font-mono sm:gap-5">
            <span className={`text-xl font-bold sm:text-3xl lg:text-4xl ${view.battle.attackPower >= view.battle.guardPower ? "text-ki-300" : "text-space-300"}`}>
              {view.battle.attackPower.toLocaleString("en")}
            </span>
            <span className="text-[10px] tracking-widest text-space-500 sm:text-xs">VS</span>
            <span className={`text-lg sm:text-2xl lg:text-3xl ${view.battle.guardPower > view.battle.attackPower ? "text-ki-300" : "text-space-300"}`}>
              {view.battle.guardPower.toLocaleString("en")}
            </span>
          </div>
          <div className="mt-1 flex items-end justify-between sm:mt-2">
            <div className="flex items-end gap-1 sm:gap-2">
              ...
              {view.them.combo.map((c) => (
                <ArenaCard key={c.id} card={c} width={32} onInspect={() => setInspect(c)} onHover={hoverOf(c)} />
              ))}
            </div>
            <div className="flex items-end gap-1 sm:gap-2">
      ...
      {/* Your side. */}
      <section className={panel} aria-label="Your side">
        <div className="flex items-end gap-1.5 overflow-x-auto pb-1 sm:gap-2 lg:gap-3">
          ...
          {view.you.battle.length === 0 && <span className="self-center text-xs text-space-500 lg:text-sm">no Battle Cards</span>}
        </div>
        <SideSummary side={view.you} />
        <EnergyRow side={view.you} onHover={hoverOf} />
      </section>

      {/* The prompt bar: the one question being asked. */}
      <section
        className={`sticky bottom-[4.5rem] z-10 flex items-center gap-2 rounded-xl border p-2 pl-3 backdrop-blur sm:bottom-2 sm:gap-3 sm:rounded-2xl sm:p-3 sm:pl-5 ${
      ...
      {/* Your hand. */}
      <section className="rounded-t-2xl border-t border-space-700 bg-space-900/95 p-2 pb-3 sm:rounded-2xl sm:border sm:border-space-700/70 sm:p-3 lg:p-4" aria-label="Your hand">
        ...
        {logOpen ? (
          ...
          <div className="flex gap-1 overflow-x-auto pb-1 sm:gap-2 lg:gap-3">
            {(view.you.hand ?? []).map((c) => (
...
function CardPreview({ card, box }: { card: CardView; box: DOMRect }) {
  ...
  const top = Math.min(Math.max(gap, box.top - 60), Math.max(gap, window.innerHeight - 560));

  return (
    <div
      className="pointer-events-none fixed z-40 hidden rounded-2xl border border-space-600 bg-space-900/95 p-3 shadow-[0_20px_60px_rgba(0,0,0,0.65)] backdrop-blur sm:block"
...
function CardDetail({ card, withName = false }: { card: CardView; withName?: boolean }) {
  return (
    <div className="space-y-1.5">
      ...
      {card.keywords.length > 0 && <p className="text-[11px] text-ki-300 sm:text-xs">{card.keywords.join(" · ")}</p>}
      {card.text && <p className="whitespace-pre-wrap text-xs leading-relaxed text-space-200 sm:text-sm">{card.text.replace(/<br\s*\/?>/gi, "\n")}</p>}
...
function PhaseStrip({ view }: { view: BoardView }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-space-500 sm:gap-2.5 sm:text-xs lg:text-sm">
      {steps.map((p) => (
...
function SideSummary({ side, them = false }: { side: SideView; them?: boolean }) {
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-space-400 sm:gap-x-5 sm:text-xs lg:text-sm">
      {them && <span className="font-semibold text-space-100">{side.name}</span>}
      <span className="flex items-center gap-1 sm:gap-1.5">
        life
        <span className="flex gap-[2px] sm:gap-[3px]">
          {Array.from({ length: 8 }, (_, i) => (
            <i key={i} className={`h-3 w-[5px] rounded-[1px] sm:h-4 sm:w-[7px] lg:h-5 lg:w-2 ${i < side.life ? "bg-gain" : "bg-space-700"}`} />
          ))}
        </span>
      </span>
      <span>
...
function EnergyRow({ side, onHover, small = false }: { side: SideView; onHover: (c: CardView) => (box: DOMRect | null) => void; small?: boolean }) {
  return (
    <div className="mt-1.5 flex items-center gap-1 sm:mt-2.5 sm:gap-1.5">
      <span className="text-[11px] text-space-500 sm:text-xs lg:text-sm">energy</span>
      {side.energy.map((c) => (
        <ArenaCard key={c.id} card={c} width={small ? 18 : 24} upsideDown onHover={onHover(c)} />
      ))}
      {side.energyMarkers > 0 && <span className="rounded bg-ki-500/20 px-1 font-mono text-[10px] text-ki-300 sm:text-xs">+{side.energyMarkers} marker</span>}
      <span className="ml-auto text-[11px] text-space-400 sm:text-xs lg:text-sm">
