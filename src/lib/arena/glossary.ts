...
export const READING_RULES: ReadingRule[] = [
  ...
  },
  {
    ...
    body: "9-1-3-3 and 20-21 both work through one sentence, and the sets print it several ways: “reduce/increase/decrease the energy or combo cost of X by N”, the possessive “this card's”/“that card's”/“its”/“their” cost, the passive “the energy cost of X is reduced by N”, “cost **on** X” beside “cost **of** X”, the plural “costs”, and a bare “cost” with no “energy”/“combo” word, which defaults to energy. All six are read as the one shape (9 Sep 2026): the possessive and passive forms are rewritten to “reduce the cost of X by N” before the shared rule runs, so a pronoun subject gets the same “this card”/“that card”/“it”/“them” handling every other rule already gives it. A named area survives that rewrite too — “this card's energy cost **in your hand**” — though it says nothing a self or pronoun reference needs, since that already finds the one card regardless of where it sits. Three costs stay unread on purpose, because none of them spell the noun as bare “cost”: a **skill's** cost (“the skill cost of …”, “the activation cost of …'s [Counter] skill”) is a number `playCost` computes with no hook to lower; a **specified** cost mixes colour and total in a way the owner has not ruled on; a **Z-Energy** cost is read raw in five call sites this op cannot reach. “X get −N combo cost” is the same reducer said the way “X get +N power” already is, and a pronoun subject (“their combo cost”) is read the same way there too. A trailing “, for each Y” is the amount's multiplier, not a second clause — split off, the first half compiles alone with a flat, unscaled number and the multiplier becomes an orphaned, separately unread fragment (BT27-123 reduced by a flat 1 rather than 1 per ≪Saiyan≫ in Warp), so the comma before “for each” no longer breaks the sentence in two. “For every N”, N > 1, is a genuine divisor no `Amount` shape can express yet (BT13-076, BT13-077) and stays unread rather than being read as “for each”, which would drop the N entirely.",
  },
  {
