...
export const READING_RULES: ReadingRule[] = [
  ...
  },
  {
    ...
    body: "9-10 says an event happens differently, or not at all — but only some events are ones this engine can stop in front of, and the `replace` operation's `event` is that closed list, not an open one. Three names are in it. **`leave`**: the card would leave the Battle Area, read where `move()` is about to take a card out of `leader`/`battle`/`unison`, and narrowed by `by` to a departure a skill caused (`\"skill\"`) or a skill or a KO (`\"skillOrKo\"`), and by `bySide` to the ones the **opponent's** skill caused — the narrowing 19 cards print, and the one `by` alone cannot draw, since a rule read as \"by a skill\" would fire on its own controller's skills too. **`ko`**: the card would be KO'd, and nothing else about it changes — a card its owner returns to hand is still returned to hand. **`play`**: the play being resolved (9-6), which only a [Counter: Play] is in time for. Nothing else is: a draw, a cost being paid, damage, a card going under another, a card leaving the **Combo** Area — the engine reaches none of those with a replacement check in front of them, so a clause that needs one is left unread rather than compiled into a rule that never fires. What happens in the event's place is a *program*, not only a destination, and that is the whole difference from `replaceLeave`, which the primitive now carries. One move of the card itself is a **redirect** — it goes there instead, the shape every wording read before 12 Sep 2026 takes. Anything else is a **substitute**: the departure does not happen at all, the card stays where it is, and the program runs with the card bound as `subject`. That is what BT3-051's “place all the cards under this card in the Drop Area instead” says, and no redirect can say it. Two things about a substitute are worth knowing. It **may** stop and ask, but only where somebody can hear it: `move()` is synchronous at 46 of its 48 call sites, and there an asking substitute is left unapplied rather than half-run, while the two sites inside `stepScript`'s `moveTo` and `ko` loops run it as a frame on the flow, which can wait for the answer (#107, 13 “you may … instead” cards). `optional` is 9-10-3's own question — whether to take the replacement at all — and it is put by the same prompt 9-10-2 uses when two replacements answer to one departure; declining leaves the departure exactly as it was. And a substitute cannot replace its own replacement: while one is running, whatever it moves is moving for real.",
  },
  {
