...
export const READING_RULES: ReadingRule[] = [
  ...
  },
  {
    ...
    body: "A prohibition may carry a budget and an escape. “Can only attack one more time” is read as one allowed use (`uses: 1`) and then a prohibition once that use is spent; “can't … unless …” carries the condition on the prohibition (`unless`) and the legality check asks it each time, so the action is refused while the condition is false and offered again when it becomes true.",
  },
  {
