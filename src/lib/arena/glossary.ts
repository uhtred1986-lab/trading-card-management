...
export const KEYWORDS: Record<KeywordSkill["name"], KeywordDoc> = {
  ...
  Servant: {
    ...
    engine:
      "Both halves, on the engine playing every game today. The power is part of the card's power everywhere it is read, so combos and comparisons see it. The rules engine (#154) reads the power half the same way (`attrBonus`); the Charge Phase half is hook group B's (`activeStep`, #155), still unbuilt there.",
    support: "engine",
  },
...
export const READING_RULES: ReadingRule[] = [
  ...
  {
    title: "Who charges a price, and from what",
    body: "Two engines now read the same record and charge it differently, and it is worth knowing which is which. The engine that plays every game today (`legacy`) charges a price in `planPayment`: one search over the Energy Area with a branch inside it for the coloured orbs, for an either-orb, for X, for the cards 20-19 lets stand in for energy, and for [Warrior of Universe 7] clearing the colours outright — every reading described on this page. The engine being built beside it (`rules`) charges the same price from **declarations**: `rulesets/dbs/costs.rules` names six prices — energy, markers, life, the card's own mode, 20-19's payers, and the printed price no engine charges itself — each saying what it `consumes:` and how the payment `asks:` its question, and `vm/costs.ts` is one planner over those words rather than a branch per price (#148). It answers the same `Requirement`s and offers the same `payCost` options, which is asserted board for board against the older engine rather than assumed. **20-21's reductions are read there too**, and read as *layers* rather than as a step in the planner: `attributes.rules` declares `costOf` as `[printed, reduction]` and `specifiedCost` as `[printed, reduction, specified]`, so a [Permanent] in play or a skill's turn-long effect lowers the total, takes one coloured orb off with each energy it removes (20-21-2), floors at zero, and relaxes a colour on its own without moving the total — the same arithmetic `playCost` runs, compared against it board for board. The one half still missing there is [Warrior of Universe 7]: it is a *keyword* rather than a cost reduction, so it is a `DEFINE KEYWORD` hook body and waits with the other twelve keyword bodies. No game is played on that engine (`ENGINE_INFO.rules.available` is false), so nothing on this page is untrue of the engine you are playing.",
  },
  {
  ...
  {
    title: "Paying with something that isn't energy",
    body: "A price is normally settled out of the Energy Area, and until 13 Sep 2026 the planner looked nowhere else. 20-19 lets a card say otherwise: “[Permanent] You can use this card to pay energy costs even when it's in your Battle Area” (BT3-039) means the card is switched to Rest Mode exactly as an energy card is, stands in for one energy of its own colours, and never moves — which is what separates it from an action price that places a card in the Drop. The compiler reads that sentence to a `payWith` step, the board keeps it as a standing permission while the card is in play, and the planner then treats the card as one more active energy card of those colours: no new kind of requirement, the same answer it already knew how to give. Two rules hold it honest. The **energy is tried first** and the wider pool only answers what the energy alone cannot, because the permission is an offer and resting a Battle Card to pay for something the Energy Area could have covered is a cost the player never agreed to. And the refusal twin counts the card too, or the board would say “1 short” about a price the menu is offering. The record can also name payers for **one price** rather than for the board, written `PAYWITH <selector> AS energy` (or `AS {Red}` for a card that counts as one orb of a colour) — the same permission, scoped to the skill whose price carries it. On the rules engine (#149) this per-price record is bound the same way an activation's marker and life amounts already are — off the record's own `payWith`, resolved against the board when the line is priced — while the whole-board [Permanent] grant above stays unread there (`vm/effects.ts`'s `DEFERRED_STATICS`); no card compiles to a per-price record yet, since the compiler only reads the [Permanent] sentence. What is deliberately **not** read is the scoped printed form: four of the five cards in this family qualify the permission — to the cards whose cost is being paid (“when paying the energy cost of blue ≪Demon Realm≫ cards in your hand”, BT28-031/032), to one play (BT17-065), or to a count per turn (BT28-106) — and the step carries no scope, so reading them would grant a permission wider than the card prints. They stay unread, which costs tokens where a wrong reading would lose games.",
  },
  {
  ...
  },
  {
    ...
    body: "“Negate its keyword skills” means the keyword stops applying (22-1-3). A line that is nothing but keyword tags — “[Deflect][Triple Attack]” — is parsed as several skills, one per keyword, so a card can negate one of them without touching the other. The sentence comes in two word orders and both are read: “negate **X's** [Energy-Exhaust] skill” and “negate the [Energy-Exhaust] skill **on** X”, the second on ten wordings that went unread until 9 Sep 2026. A tag that names a *kind* of skill rather than a keyword — “negate the [Auto] skills of …” — is let past both rules rather than turned into a keyword that does not exist; that word order is still unread. Underneath, every negation the compiler writes — a card's skills, one kind of them, one keyword, the skill itself — is one `negate` step of the rules language (`docs/arena-ruleset-spec.md` §2.2, since 14 Sep 2026), run on both engines as the spelling it stands for; the four spellings stay what the compiler writes and what a record holds, so no card's reading moved when the primitive arrived.",
  },
  {
    title: "A skill copied from another card",
    body: "20-18. Ten cards say one card takes on another's printed skills, in two wordings, and both are read as one \u201ccopySkills\u201d step. \u201cThis card gains all of the chosen card's keyword skills for the turn\u201d (BT26-139) and \u201cGain all of the chosen card's skills\u201d (BT3-049) name the source outright. The commoner wording is printed as two clauses \u2014 \u201cChoose up to 1 keyword skill on a card placed under this card\u201d and \u201cand this card gains that skill until the end of your opponent's next turn\u201d (BT20-028) \u2014 and neither half means anything alone: read apart, the first compiled as a choice of *cards* and the second went unread, so the two are joined before the step is read. The choice is among the **skills** of every card the phrase finds, not among the cards, and the engine asks it as the skill resolves, with \u201cNone\u201d as the last option because every such card prints \u201cup to 1\u201d. What is copied is the printed face as it stood when the copy was made (9-9), so it survives the source being flipped, silenced or leaving play. A copied **pure keyword** skill is granted as a keyword and behaves exactly like one the card prints (20-18-1 writes them the same way); anything else is carried on the target as a copy, enumerated with its own skills \u2014 so a copied [Auto] answers to the *target's* moments, a copied [Permanent] stands on the target, and \u201cnegate that card's skills\u201d silences the copies with the rest. Two cards stay unread on purpose: BT20-088 copies a skill off the card it has just KO'd, which is a source that has left play, and BT22-037 chooses one keyword skill, negates it on its owner, and grants that same skill to a card chosen afterwards \u2014 one chosen skill read twice, which this step cannot say. Sixteen more cards print the skill they give **in full and in quotes** (\u201cgains \u201c[Auto] When this card attacks, choose up to 1 of your opponent's Battle Cards and it gets \u221225000 power for the turn\u201d\u201d, BT18-008), which 20-18-1 writes the same way and which is not built yet. The decision, so it is not taken twice: it belongs to this step as an inline program, not to \u201cgrant\u201d as a text field \u2014 \u201cgrant\u201d makes a keyword effect and a quoted [Auto] is not a keyword, while a quoted skill needs exactly what a copy needs, a skill the target enumerates and resolves although its own face does not print it. What it adds is only where the program comes from: a skill written on the granting card rather than snapshotted off another one.",
  },
  {
  ...
  {
    title: "A specified cost, and the baseline it needs",
    body: "A play's price has two halves. The **total** is the number in the cost circle — or X, which the card's master picks when they play it (1-2-2-2-1). The **specified cost** is the coloured orbs beside it: how much of that total has to be paid with energy of a named colour. `costReduction` with `what: \"specified\"` is the only op that moves the second half, and by the owner's ruling on BT19-039 (9 Sep 2026, read off 13-2-1-3 and 20-21-2) it moves *nothing else*: “reduce the specified cost of this card in your hand by {u}” takes BT19-039's two blue orbs down to one and leaves the total exactly where it was. The orbs are carried on the op as printed — `{u}`, `{y}{y}` — never as a bare count, because relaxing blue and relaxing yellow are different changes. A Unison then arrives with markers equal to the **total** energy rested (13-2-1-3), specified or not, so a relaxed colour requirement lets a player pay *more* colourlessly rather than paying less; paying less arrives with fewer markers, which on BT19-039 is the difference between meeting its own “if this card has 3 or more markers” and not. [Warrior of Universe 7] (22-19) is the same half said absolutely: it clears the specified cost of ≪Universe 7≫ cards and never touches a total either. **Where the engine stops is the baseline.** A fixed cost's orbs are filled by convention (`specifiedCostOf`: one of each of the card's colours, capped by the total), but an X cost has no total to cap, and the catalog has no field for the orbs at all — checked card by card on 12 Sep 2026, and `npm run arena:specified` re-checks it and lists what is waiting. All 167 X-cost cards of the original game are Unisons or Z-Unisons, and seven of them print a specified-cost clause (BT19-039, BT19-040, BT15-063, BT20-118, P-673, P-600, BT25-004). BT19-039 is why the convention is not stretched over the gap: its printed requirement is **two** blue and one-orb-per-colour would say one, so the engine answers “no colour demanded” and charges the total alone — lenient, and visibly so, rather than wrong in a way nothing would catch. A card whose orbs are actually known says so on its def (`specifiedCost`) and the whole mechanism runs off it: the menu offers only the values of X whose colours can be met, the payment planner rests the right energy, the refusal names the colour it is short of, and the row's price shows the relaxed requirement instead of the printed one. **Where the orbs come from is a person** (issue #255, owner's decision of 13 Sep 2026): the `cards.specified_cost` column, in this same orb notation, entered from the rules record on the workbench — which says *incomplete: specified cost unknown* on every X-cost card still without one — or seeded by a migration on a ruling (BT19-039 is entered as `{u}{u}`). The catalog upsert coalesces the column so a sync never erases an entry, `npm run arena:specified` reports per card whether the baseline is entered, ruled on or still unknown, and the probe's *reduced* board plays such a card from hand on exactly the energy its relaxed requirement leaves — one blue for BT19-039 — and says whether the play would have been refused without the rule.",
  },
  {
    title: "Counted and conditional prohibitions",
    body: "A prohibition may carry a budget and an escape. “Can only attack one more time” is read as one allowed use (`uses: 1`) and then a prohibition once that use is spent; “can't … unless …” carries the condition on the prohibition (`unless`) and the legality check asks it each time, so the action is refused while the condition is false and offered again when it becomes true. Both are asked by `legalActions` and by the `whyNot*` twins from the one predicate, so the menu and the refusal cannot disagree. Two things the shapes turn on. A **budget is spent by the action happening**, not by the prohibition being read: `apply` decrements it once the move is taken, so a `uses` rule left unread never runs down, and a rule with uses remaining forbids nothing. An **escape clause is a sentence of the card that printed it**, so it is asked in *that* card's chair (the prohibition records its `master`) — reading “your opponent” from the chair of whoever is trying to act would invert every such card — and it is then said back to the player being refused in their own words, which is the one place the engine mirrors “you” and “your opponent” rather than printing them as the script wrote them. An escape the compiler cannot read takes the **whole** prohibition with it rather than leaving the half before “unless” standing: “your opponent can't attack for the turn unless they discard 1 card each time” is a cost the opponent may choose to pay (replacement territory, out of scope here), and reading only the ban is a stricter rule than the card prints. Twenty clauses of the catalog read that way before this — BT10-063, BT11-053, BT13-082, BT13-140 among them — so the compile rate falls by 15 cards where it used to over-forbid. **Both engines read this the same way.** The rules engine being built beside the one you are playing reads 20-14 off the board too — a [Permanent] in play, a skill's turn-long effect, and a card's own rule about itself wherever it sits (9-1-3-3, which is how “this card can't be played from any area except by skills” holds in a Drop Area) — through the same `Prohibition` shape, the same budget, the same escape clause read in the printing card's chair, and the same mirrored words when it is said back. There a move is gated on it by a `forbidden()` line in the move's own paragraph rather than by a hand-written twin, and `scripts/verify/vm.ts` §21 stages one on both engines and compares the refusal.",
  },
  {
  ...
  },
  {
    ...
    body: "A rule that reads a clause is only as good as the target grammar underneath it, and two shapes are known to go further than that grammar can follow. “Your opponent's **Leader**” comes back as *any one card in their play area* rather than the Leader, and “all **other** Battle Cards” comes back as all of *your own*, this card included, rather than every Battle Card but this one. Both are silent: the clause compiles, reads plausibly, and aims at the wrong cards. Because of them, “negate the skills **of** X” is deliberately left unread on the seven cards that print it — seven unread beats three read wrongly (ground rule 5), and the fix belongs in the target grammar, where a Leader becomes a special target and “other” becomes the exclusion it already has a field for. A third shape is now refused outright rather than read loosely: “play up to 1 \<A\> card **and** 1 \<B\> card —both green— **from your deck**” names two *different* cards drawn from one shared source, and no primitive says that. Split at the “and”, the first name lost its source and read as a card already on the board, while the second took the source and had its side flipped by a stray “their” in “with their skills negated”. Read instead as one filter, “A and B” collapses to “A or B” — satisfied by either alone, fewer cards than printed, still not what the card says. Twelve cards left the fully-read count when it was refused, and every one of them had counted as complete only because the wrong reading hid the gap (BT20-077, BT20-079, BT24-123, EB1-32, EX23-37). A fourth is refused for the same reason: “choose **the rest of** your and your opponent's cards in the Battle Area” names the cards of a description *except the ones this skill has already picked out*, and a selector has no way to subtract the last choice from itself — so read whole it sweeps away the very card TB3-066's option chose to spare. Only in a target phrase: “for the rest of the turn” is a duration and never names a card, and “the rest” of a pool a look held out is what was looked at minus what was chosen, which the move already says (BT3-062).",
  },
  {
    title: "For each marker, not for each card",
    body: "“For each marker on X” (13-2) counts the markers on the cards X names, added up — a different question from “for each of X”, which counts the cards themselves. Read as a card count, “for each marker on this card” asked how many cards are named *this card*, which is always exactly one: BT27-003/004/005/006 printed a flat +5000 power that never grew with the counter it names, and BT15-120 gave +5000 for every Unison Card the opponent had rather than for every marker on them. The count more often trails the effect it multiplies (“this card gets +5000 power for each marker on it”), but on the four BT27 cards and EX19-21 it leads the sentence instead, comma-joined — the reverse of a trailing duration, and read the same way: reattached to the effect before either is compiled, rather than left as two clauses that cannot see each other. EX19-21 prints the same leading count over a pronoun (“it gets +5000 power”) that the engine cannot resolve on a fresh [Permanent], and stays unread rather than answered — the leftover “this card” in “for each marker on this card, it” is exactly the substring a subject-phrase match would otherwise seize on, so a clause that fails to compile once its count is known to be a marker total refuses outright instead of falling through to a pattern that cannot tell that phrase from its own subject. A reduced cost is not read this way yet: BT15-095, BT24-139 and BT28-054 print the same “for each marker on …” but lower an energy cost rather than raise a power, which the language can now express (`markers` is one `Amount` beside `count`) but nothing yet compiles into — left split and unread, as before, rather than merged into a clause that only changes which text shows up unread. Two counted phrases joined the reading on 12 Sep 2026, both narrow and both local to this rule rather than to the target grammar at large. “For each marker on **it**” is the pronoun the sentence has been using for this card all along (P-377, P-378, DB3-144, whose whole skill it is); the target grammar reads “this card” and not the bare pronoun, and teaching it the pronoun everywhere would change every clause that merely ends in “it”, so the pronoun is read only where the phrase is already known to be what is counted. “For each **1 energy you have**” (TB1-038, BT1-030 twice) names the area as its noun, with the “1” saying the size of each step rather than a multiplier — “for each 2 energy” would mean dividing the count, which no amount can do, and is left unread rather than read as the same thing.",
  },
  {
    title: "An amount is an expression",
    body: "A number an effect needs is no longer a short list of shapes but a small expression tree (20-5, 12 Sep 2026). It may still be a printed number, how many cards a name holds (`$t`), a count of what a selector finds (`count(SEL)`), the markers on them (`markers(SEL)`), the total power of named cards (`sumPower($t)`) or however many a hand is short of a figure (`handUpTo(N)`) \u2014 every one of those is spelled exactly as it always was, because stored `card_rules` rows carry those keys and a rename would change what a saved rule means. Four shapes are new: `X`, the value this skill's price was paid at; `life(you|opponent|both)`, the life cards of one side or of both; `attr(TARGET, name)`, one card's own power, combo power, energy cost or combo cost; and `sumOf(SEL, name)`, that same measure over every card a selector finds, added up \u2014 `sumPower` is now the special case of the last of those that the rows already spell. Any of them may be multiplied (`* 1000`) and any expression may have a printed number added to it (`+ 1`). **The right-hand side of both operators is a printed number, never a second reading of the board**: no card multiplies one count by another, and allowing it would make the printed form ambiguous about which came first. Of the new shapes, only `X` is produced by a wording so far \u2014 `life`, `attr` and `sumOf` are the language's, for a rule written by hand or by the referee, and no printed line compiles into them yet. An amount is read in exactly one place in the engine, so what a shape means is settled once rather than per operation.",
  },
  {
    title: "X, paid once and read again",
    body: "“{X}” is a price whose size the player chooses when the skill is activated, and the effect after the colon may say `X` and mean that same number (20-5). The engine offers the skill **once per value of X it can actually pay** \u2014 the same shape it already used for playing an X-cost card \u2014 so the choice is made on the move list rather than through a prompt of its own, and an X the energy cannot settle is never on the menu. The value is charged alongside the skill's own orbs and carried onto the running program, where `X` reads it. “Pay X energy” is the same price written out in words and is read the same way. A `choose` step may bind X instead, to how many cards were taken, for the prices that are a choice rather than an energy payment \u2014 and that number crosses from the price into the effect on a key of its own beside the *names* the price bound, because the continuation those names travel on is read back by a game saved in the counter window between a price and its effect. Under one program, `bindX` binds only for the steps after it. Three guards keep this honest. A program that says `X` with nothing to bind it **fails validation** and can never be stored \u2014 read as zero, “draw X cards” would be a skill that costs nothing and does nothing, which looks like it worked; the check is made in order, so a step before the `choose` that binds X is refused while one after it is allowed. Until 12 Sep 2026 “{X}” stripped to nothing and read as a **free** skill, because the orb reader ignores the letter and the emptiness that was left counted as “no price at all”. And no bounds are read off a card: none prints one, and inventing a ceiling would refuse a payment the rules allow.",
  },
  {
    title: "A card no skill may touch",
    body: "“This card isn't affected by your opponent's skills” (9-1-4) is read into its own operation rather than folded into a prohibition, because it is a stronger claim than “can't be chosen” — the rule covers everything a skill might do to the card, not only the moment of choosing it. **Which skills it refuses is the rule's own business, never the board's**: a stored immunity carries the player whose skills it blocks (`from`, resolved against the card's controller when the rule is made) and a filter on the cards doing the affecting (`fromFilter`), and the check asks that pair — this card, that skill — of every chooser, the card's own controller included. Reading the side off who owns the card instead, as the engine did until 13 Sep 2026, can only ever say one of the three things the family prints: “Non-<Gogeta: GT> skills” (BT18-019) names no side at all, and by the same convention as “all other Battle Cards” meaning both Battle Areas, that means every skill's, the card's own side's included — which the owner test silently narrowed to the opponent's. “The skills of opponent's cards other than Battle Cards” and “…non-Extra cards” narrow the *source*, which the plain wording never needs to say — a filter on the card doing the affecting, not the card being affected; a skill with no card behind it is therefore not blocked by a `fromFilter` rule, having nothing to match. **Where it is enforced**: once, in `resolveSelector` — the one place an op reaches a card, beside [Barrier] and “can't be chosen by skills” — so a new operation inherits the check instead of repeating it. The one selector kept out of it is `self`: a card's own skill naming itself is the skill working, not a skill touching the card from outside. That covers being chosen, and also the sweeps and the board-wide power changes, which name their cards through a selector like everything else; what is still **not** caught is an effect that names no card at all — a skill aimed at a player, damage, a rule of the game. That gap is written down rather than closed by guessing, and is what the family's own probe reports: staged against an opponent's KO skill, a working reading shows the card was never among the targets offered. A [Permanent] immunity is in force against another [Permanent] as well as against an effect with a duration: the static layer answers nothing one level down to keep selectors from looping, and immunity is the one kind exempt from that (`immunityStatics`), because it decides whether a static lands at all. Refused, the card is not simply missing from the menu — `rejectedActions` answers with the immunity itself, said in the refused player's words, so the rule its controller reads as “your opponent's skills” reads back as “your skills”. One duration has no exact answer: “until the start of your next Main Phase” (BT9-119) is a phase later than any duration this engine can name, and is left unread rather than approximated.",
  },
  {
    title: "Taking control of a card",
    body: "20-9-1 defines gaining control as moving another player's card to your area and becoming its master, and that is exactly what the operation does — there is no controller field, because in this engine the area a card is in already *is* who masters it (0-3-4-1, and the audit above). What follows from reading it that way is that most of control was already true: a card in your Battle Area is on your attack menu (8-1), is one of *your* cards to every selector and every trigger's “your cards in play”, and answers to your [Permanent]s — none of which needed a line of code, and all of which would have needed one had control been a flag beside the area. What did need saying is the three places the card's **owner** still decides. A KO sends it to its owner's Drop Area (5-12-1), never to its new master's. It keeps its mode, its markers and every continuous effect on it (20-9-2), so the move carries rather than resets — 3-1-4-1 names gaining control as one of the two moves that do — and it keeps the turn it entered play, because a card changing hands has not been played again. And a duration is a **loan**: the continuous effect it registers carries the way home and expiring it walks the card back, so “until the end of the turn” ends in the cleanup with the card where it started. A loan whose card has since been KO'd or bounced is simply gone, because a card leaving play drops the effects on it; a loan whose card sits in a Combo Area is not returned from there either, and says nothing rather than teleporting it. Two refusals are deliberate. **A Leader and a Unison Card can't change hands**: the manual gives no route to taking one, and those areas hold one card each, so the operation logs a note rather than quietly doing nothing. And a card outside a Battle Area cannot be taken at all — 20-9-1's “move to your area” has no meaning for a card in a hand or a Drop. Nine cards print the wording and five read: “gain control of it” after a choice that bound one (BT15-118), “gain control of this card” (P-413) and “your opponent gains control of this card” (BT19-055, P-277, P-564) — with or without a printed duration, and where none is printed the operation carries none, because control that quietly ended at the End Phase would be a different card from one that says nothing about giving it back. **The other four are refused on purpose, and two of them are the reason this reading is narrower than the sentence.** `refFor` falls back to *this card* for a pronoun with nothing else to resolve against, and to the last *choice* for “the played card” with nothing playing — both sensible defaults everywhere else and both wrong here, because a card cannot be taken from the player already mastering it and because the last choice is as often the price as the prize. BT21-063's “you gain control of it” follows “place this card under that card”, so the fallback pointed at the one card the pronoun cannot mean; BT19-154's “gain control of the played card” would have taken the Battle Card you had just sent to your own Warp to pay for it. Neither showed up as a coverage number moving the wrong way — both were found by diffing `arena:readings`, which is what that instrument is for. So self is accepted only where the print says “this card”, and “the played card” only where something was played. The remaining two are unread for the plainer reason that the clause which would have bound their pronoun is itself unread: P-348's “if an opponent's Battle Card would be KO'd **by this card's attack**” is a replacement moment the engine does not reach, and BT16-115 counts what the opponent chose to send away. A pronoun answered by guessing is worse than a clause left open.",
  },
  {
    title: "Skipping a phase or a step",
    body: "20-13 is short and total: a skipped phase or step is *not performed*, no events occur inside it, no trigger condition such as “at the start” or “at the end” is met (20-13-2), no action can be declared or carried out (20-13-3), and no checkpoint happens (20-13-4) — while any continuous effect that was to end in it ends as it is skipped (20-13-5). Nothing else in the language says that: an `if` skips a *program*, not the turn's own steps, and a prohibition (20-14) refuses an action a player would declare rather than stopping the phase they would declare it in. So `skip` is a flag rather than a jump: the operation writes it down and the flow runner spends it where the step would begin, which is the one place all five of those clauses can be applied at once. **What can be skipped is a closed list of five**: the Charge, Main and End Phases, and the Offense and Defense Steps of a battle. Not a whole turn — 20-13's other half — because a turn in progress has no single point in the flow to be refused at, and not the Damage Step, which is where a battle's result is worked out rather than a moment anyone acts in. Skipping the Charge Phase is the largest of the five and the one worth knowing in detail: 7-2-7's Active Step and 7-2-9's draw are both inside it, so a skipped Charge Phase leaves the turn player's rested cards rested and their hand as it was, which is the point of the wordings that ask for it. The turn-relative effects that expire at 7-2-4 are deliberately *outside*, because 20-13-5 says they end when the phase is skipped rather than outliving it; the End Phase is the same, and keeps its cleanup for the same reason. One entry is spent by one occurrence, and *which* occurrence is `when`: “this” is the one in the turn the skill resolved on, “next” the first in a later turn — which is what a card played in your own Main Phase means by “your next Charge Phase”, that phase having already gone. An unspent “this” entry is dropped when the turn passes rather than eating the next turn's phase, the same way a delayed effect that missed its moment is. **All four cards that print the word compile now (#278), and none of them is the one-shot flag alone.** `what` grew two members rather than gaining a sixth kind of one-shot occurrence. `\"turn\"` (BT31-097, “Skip your turn and begin your opponent's Charge Phase”) refuses the player's *whole next turn* — every phase of it at once — checked at that turn's own start rather than at any one phase, which is the single point in the flow a turn in progress never had; the turn still counts for turn-number bookkeeping; `turn.next` advances `s.turn` once per transition whether or not the turn it is leaving did anything (owner's ruling, 14 Sep 2026). `\"span\"` (BT21-104, “skip all phases until the Charge Phase in your next turn, then start your Main Phase”) is three ordinary entries under one name rather than a mechanism of its own — the rest of the turn this resolves in (`end`, `this`), the opponent's whole next turn (`turn`, `next`), and this player's own next Charge Phase (`charge`, `next`) — so it needed no check point beyond the one `\"turn\"` already added. BT18-001 and BT18-019 print theirs as [Permanent]s conditioned on a battle in progress (“when this card is in a battle”, “when your … cards attack …”), which is a standing rule rather than a one-shot flag: `collectStatics` reads a `skip` op inside a [Permanent] into a `StaticEffect` of kind `\"skip\"`, exactly as it already does for `forbid`, and `stepSkippedByPermanent` asks it live at `battleOffense`/`battleDefense`, alongside `takeSkip` rather than instead of it — the one-shot entry and the standing rule are the same 20-13 clause read two ways, never both on the same card. `parseConditionClause`'s active-voice “X attacks Y” (`role: \"attacker\"`) is what BT18-001's own condition needed beside the passive “is attacking” it already read.",
  },
  {
    title: "What a replacement effect can replace",
    body: "9-10 says an event happens differently, or not at all — but only some events are ones this engine can stop in front of, and the `replace` operation's `event` is that closed list, not an open one. Four names are in it. **`leave`**: the card would leave the Battle Area, read where `move()` is about to take a card out of `leader`/`battle`/`unison`, and narrowed by `by` to a departure a skill caused (`\"skill\"`) or a skill or a KO (`\"skillOrKo\"`), and by `bySide` to the ones the **opponent's** skill caused — the narrowing 19 cards print, and the one `by` alone cannot draw, since a rule read as \"by a skill\" would fire on its own controller's skills too. **`ko`**: the card would be KO'd, and nothing else about it changes — a card its owner returns to hand is still returned to hand. **`play`**: the play being resolved (9-6), which only a [Counter: Play] is in time for. **`life`** (#272): a life card's own move to the hand or the Drop Area — 8-4-6-1's damage, not a Battle Area departure at all, and narrowed by `to` rather than `by`/`bySide`, which mean nothing here since nobody's skill puts a card out of the life area. `move()`'s replacement lookup, gated to a card leaving `leader`/`battle`/`unison` for the other three events, is separately gated to a card leaving `life` for this one — the two gates share the redirect-or-substitute logic below but nothing else, and neither widens the other. BT10-031 and SD18-01 (\"you may reveal it and add it to your hand instead\") are the only two cards that print it, found through `battleDamage`, the one life-departure site among five (`docs/arena-life-card-replacement-scope.md` §1.1) that can suspend for their `optional` question — the other four stay exactly as they were. Nothing else is reached at all: a draw, a cost being paid, a card going under another, a card leaving the **Combo** Area — the engine reaches none of those with a replacement check in front of them, so a clause that needs one is left unread rather than compiled into a rule that never fires. What happens in the event's place is a *program*, not only a destination, and that is the whole difference from `replaceLeave`, which the primitive now carries. One move of the card itself is a **redirect** — it goes there instead, the shape every wording read before 12 Sep 2026 takes; a `life` event is always a substitute in the two cards that print it, since revealing takes an op of its own beside the move. Anything else is a **substitute**: the departure does not happen at all, the card stays where it is, and the program runs with the card bound as `subject`. That is what BT3-051's “place all the cards under this card in the Drop Area instead” says, and no redirect can say it; BT10-031/SD18-01's own substitute — `reveal` then `moveTo … faceUp: true` — carries the reveal onto the arriving card's face so it stays public in a hand the way a face-up life card already is (`revealedTo`, `view.ts`). Two things about a substitute are worth knowing. It **may** stop and ask, but only where somebody can hear it: `move()` is synchronous at every other call site, and there an asking substitute is left unapplied rather than half-run, while `stepScript`'s `moveTo`/`ko` loops and `battleDamage`'s own life-card loop run it as a frame or a paused flow step, either of which can wait for the answer (#107, #272). `optional` is 9-10-3's own question — whether to take the replacement at all — and it is put by the same prompt 9-10-2 uses when two replacements answer to one departure; declining leaves the departure exactly as it was. And a substitute cannot replace its own replacement: while one is running, whatever it moves is moving for real.",
  },
  {
  ...
  {
    title: "A card's power as a condition",
    body: "The engine asks it of one selector — this card, your Leader, your opponent's Leader — against a bound. The sets print the question four ways and only one of them was read: “if this card's **power is** 30000 or more”. The other three move the verb or the measure without changing the question — “if this card **has** 20000 **power or more**”, “if your opponent's Leader Card has 10000 **or less power**” — and seven skills went unread for that. An unread condition takes its clause with it, so BT28-003 and EX25-31, whose whole printed text is the condition and the grant it guards, read as nothing at all. Two subjects stay unread on purpose: “**its** power is 20000 or more” and “**that card** has 25000 power or less” measure a card an earlier clause named, and a condition is parsed with no antecedent to resolve them against; “**you or your opponent's** Leader Card has 15000 or more power” asks about either Leader, which this condition states of one selector at a time.",
  },
  {
    title: "Both players' turns as one moment",
    body: "“At the start of you and your opponent's Main Phases”, “at the end of you and your opponent's turns”, “at the end of each player's turn”: one moment in the text and two in the engine, which reads whose turn it is as a rule of its own (7-1). Neither wording matched the one-sided phrase each trigger was anchored on, so twelve [Auto] skills answered to no trigger at all and never fired once — a card that stands itself up every Main Phase simply stayed rested. Nothing said so either: a timing clause is consumed whether or not a trigger was found for it, so these carried no unread clause and counted as no gap, which is why only a card-by-card reading found them. Such a skill now answers to both triggers, and only one of them can happen at a time, so it fires once. EX24-20 prints the phrase at the end of its sentence instead of the head, so the trailing-trigger phrase admits it too. The one-sided wordings are untouched: “at the end of your turn” still answers to your turn and not your opponent's.",
  },
  {
    title: "Both players named as one owner",
    body: "“Choose up to 1 of you or your opponent's Battle Cards”, “1 card in your or your opponent's Battle Area”, “all of your and your opponent's Battle Cards”: one description of cards, owned on both sides of the table. The possessive nearest the area word is the opponent's, so reading the side off it narrowed nine such choices to the opponent's board alone and left the card on your own side — which the text plainly offers — off the menu; BT4-045 asks in its next clause whether the card chosen “was your own Battle Card”, which could never be true. The phrase is now read as both sides wherever it appears. Only the form carrying a possessive after the joiner counts: “you and your opponent draw 1 card” names two players acting, not one set of cards, and is read elsewhere. The “and” of that phrase is not a sentence break either: cut there, “choose all of you” is a choice with no cards in it and “your opponent's Battle Cards” a description with no verb, which is how fifteen sweeps were lost — EX13-07 keeping its tail and reading as your own Leader taking -10000 power and nothing else.",
  },
  {
    title: "Original power, and originally skill-less",
    body: "20-3-1: “original” is what a card describes before any skill effect has applied — the printed face, not its current, possibly modified, state. `P-295`, `BT11-152`, `DB1-041` and thirty-three more print “an original power of N”; `BT22-081`, `BT22-084`, `BT22-085`, `BT22-088`, `BT22-089`, `BT22-100`, `BT23-106`, `BT25-001b`, `BT25-009`, `EX25-02`, `P-529` and `P-692` print “an originally skill-less Battle Card” — printed with no text at all (1-5-9), whether or not it has since gained a skill (9-1-4). Neither had a filter field to land on, and both compiled cleanly and read wrongly: `parseFilter` had no pattern for “power **of** N” at all — only the sets' other word order, “N power” — so the whole measure vanished from a filter that otherwise read fine, exactly the “compiles and reads wrongly” failure this page exists to catch; “originally skill-less” had no pattern whatsoever and vanished the same way. The fix is `originalPowerMin`/`originalPowerMax` and `originallySkillLess` on `CardFilter` (`engine/filters.ts`), read off `d.power`/`d.skill` — the printed face, since nothing in `cardNow` ever rewrites either, so these two fields cannot read the *current* number or text even if a card asked for it (that gap belongs to `powerMin`/`powerMax` and to bare, non-“originally”, “skill-less”, and stays open). A second bug hid behind the first: `filterFor`'s `narrows` check — the list that decides whether a filter narrows anything or is thrown away as noise — had no entry for either new field, so “you have an originally skill-less Battle Card in play” (`P-529`, no other qualifier) built a filter, then discarded it as saying nothing, the same silent widening `BT19-130`'s character-name phrase suffered before it was added to that list. Both fields are on it now. `attr(REF, originalPower)` reads the same printed number as a value rather than a filter, for “its power becomes the attacking card's original power” (`BT19-129`) — that whole mechanism (power *becomes* a value, rather than changing by an amount) has no op yet and stays unread; the expression exists for the referee to rule with by hand. Continuous effects that have already changed a card's colour or traits (20-1) are a different question and stay out of this fix.",
  },
];
