...
function parseCountCondition(t: string): Cond | null {
  ...
  // "You have **only** 3 or less cards other than this card in your hand": the
  // adverb sits where the number is expected, so the count was not read at all
  // and the whole tail became the description — which then failed and left
  // `atLeast: 1`, turning a gate that asks for a nearly empty hand into one that
  // holds almost always. Two cards, and on BT2-006 the leftover description came
  // out as "this card", so its energy discount read as unconditional.
  const m = /^(?:you have|your opponent has|there (?:are|is)) (?:only )?(?:(no)|(?:an?|any) |(\d+) or (more|less|fewer) )?(.+)$/.exec(t);
  if (!m) return null;
