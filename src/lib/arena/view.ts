...
function effectsOn(ctx: EngineContext, s: GameState, id: string, statics: StaticEffect[]): EffectView[] {
  ...
  for (const e of statics) if (e.target === id) out.push(effectView(ctx, s, describeStatic(e, s.cards[e.source]?.owner ?? null), "permanent", e.source, s.cards[e.source]?.owner ?? null));
  return out;
}
...
function rulesOn(ctx: EngineContext, s: GameState, p: PlayerId, statics: StaticEffect[]): EffectView[] {
  ...
  for (const e of statics)
    if (!e.target && e.kind === "forbid" && about((e.value as { player?: PlayerId }).player)) out.push(effectView(ctx, s, describeStatic(e, s.cards[e.source]?.owner ?? null), "permanent", e.source, s.cards[e.source]?.owner ?? null));
  return out;
}
