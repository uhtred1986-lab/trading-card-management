...
export const DeckDraftSchema = z.object({
  ...
  zDeck: z.array(z.object({ cardId: z.string(), quantity: z.number().int().min(1).max(4) })).max(8).describe("Z-Deck cards (Z-BATTLE/Z-EXTRA/Z-UNISON/Z-LEADER), up to 8 total; empty if none fit"),
...
export async function buildPools(db: Db, leaderId: string): Promise<{ leader: typeof cards.$inferSelect; owned: PoolCard[]; buy: PoolCard[] }> {
  ...
  const buy = await db
    ...
    .where(and(...onColour, ownedIds.length ? notInArray(cards.id, ownedIds) : undefined, sql`(${cardSets.line} = 'masters' or ${cards.setCode} = ${leader.setCode})`, ne(cards.rarityCode, "PR")))
    .orderBy(desc(cardSets.sortKey), asc(cards.id))
    .limit(BUY_CAP);

  return { leader, owned, buy: buy.map((c) => ({ ...c, owned: 0 })) };
}
...
export function sanitiseDraft(draft: DeckDraft, pool: Map<string, PoolCard>): SanitisedDraft {
  ...
  const z = merge(draft.zDeck, true).slice(0, RULES.zMax);
...
export async function suggestDeck(db: Db, leaderId: string): Promise<{ deckId: number; draft: DeckDraft; sanitised: SanitisedDraft }> {
  ...
  const { output: draft } = await recordRun<DeckDraft>(db, "deck_wizard", { leaderId, mode: "build", ownedPool: owned.length, buyPool: buy.length }, res);
  const sanitised = sanitiseDraft(draft, pool);
  ...
  const [deck] = await db.insert(decks).values({ name: `${draft.name} (Claude draft)`, description }).returning({ id: decks.id });
  const values = [
